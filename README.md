## [Completed Card Generator](https://meguerreroa.github.io/FGO-Card-Generator/fgo.html)
Contains every card frame from meguerreroa's generator.
updated with more cards 
including grand,forigner,pretender and more beast

## Background
Forked from meguerreroa's fgo generator and
Several projects that experiment on HTML5 canvas which is an expansion of juncaixinchi's original concept of using canvas to generate FGO cards. 


meguerreroa's generator can be found [here](https://meguerreroa.github.io/FGO-Card-Generator/fgo.html) which is updated from 
Aister's generator which can be found [here](https://aister.github.io/FGO/fgo.html) and the 
original generator can be found [here](https://juncaixinchi.github.io/FGO/fgo.html)

Original source code can be found [here](https://github.com/juncaixinchi/FGO)
